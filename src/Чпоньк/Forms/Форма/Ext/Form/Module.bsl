...
Асинх Функция ПрочитатьФайлВерсий(Приемник)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Ключ = СтрШаблон("%1.%2", Класс, ИмяОбъекта);
		
		Если ЧастиИмени.Количество() = 4 Тогда
			
			Класс = ПоРусски[ЧастиИмени[2]];
			ИмяОбъекта = ЧастиИмени[3];
			Ключ = СтрШаблон("%1.%2", Ключ, ИмяОбъекта);
...
Асинх Функция СформироватьФайлыРасширения()
	...
	ДополнитьДанныеЗаполненияОписаниемКонфигурации(ДанныеЗаполнения, КлючиПодчиненныхОбъектов);
...
Функция ДанныеЗаполненияДляВыбранныхМетодов()
	...
	Для Каждого Метод Из ВыбранныеМетоды Цикл
		...
		Если ЧастиИмениМодуля.Количество() = 1 Тогда
			...
			Класс = "ОбщиеМодули";
			ИмяОбъекта = ЧастиИмениМодуля[0];
			
		ИначеЕсли ЧастиИмениМодуля.Количество() = 2 Тогда
			
			Класс = ЧастиИмениМодуля[0];
			ИмяОбъекта = "";
			Модуль = ЧастиИмениМодуля[1];
			
		Иначе
			
			Класс = ЧастиИмениМодуля[0];
			ИмяОбъекта = ЧастиИмениМодуля[1];
			Модуль = ЧастиИмениМодуля[2];
			
		КонецЕсли;
		
		КлючОбъекта = СтрШаблон("%1.%2", Класс, ИмяОбъекта);
		ОписаниеОбъекта = ДанныеЗаполнения[КлючОбъекта];
		
		Если ОписаниеОбъекта = Неопределено Тогда
			ОписаниеОбъекта = НовоеОписаниеОбъекта(Класс, ИмяОбъекта);
		КонецЕсли;
		...
		Если ИмяФайлаМодуля = Неопределено Тогда
			...
			Если ОписаниеПодчиненногоОбъекта = Неопределено Тогда
				...
				КлючПодчиненногоОбъекта = СтрШаблон("%1.%2.%3", Класс, ИмяОбъекта, Модуль);
				ОписаниеПодчиненногоОбъекта = НовоеОписаниеПодчиненногоОбъекта(Класс, ИмяОбъекта, Модуль);
			...
			Если ОписаниеПодчиненногоОбъекта.Тип = "Форма" Тогда
				...
				Если ЧастиИмениМодуля.Количество() = 4 Тогда
					...
					Если ОписаниеЭлементаФормы = Неопределено Тогда
						...
						Форма = ПолучитьФорму(СтрШаблон("%1.%2.Форма.%3", ВЕдинственномЧисле[Класс], ИмяОбъекта, Модуль));
	...
	Для Каждого ОписаниеОбъекта Из ДанныеЗаполнения Цикл
		...
		Если Не ОписаниеОбъекта.Значение.ДанныеЗаполнения.Свойство("ПодчиненныеОбъекты") Тогда
			Продолжить;
		КонецЕсли;
...
&НаКлиенте
Функция НовоеОписаниеОбъекта(Класс, ИмяОбъекта)
	
	ОписаниеКласса = ОписаниеКласса(Класс);
	ФайлОписания = СтрШаблон("%1%2%3",
		ОписаниеКласса.БазовыйПуть,
		ЗначениеНастройки("РазделительДиректорий"),
		СтрЗаменить(ОписаниеКласса.ФайлОписания, "{Объект}", ИмяОбъекта));
	
	ДанныеЗаполнения = ДанныеЗаполненияОбъекта(ОписаниеКласса, ИмяОбъекта);
	
	Если ДанныеЗаполнения.Свойство("ИдентификаторРасширяемогоОбъекта") Тогда
		
		Ключ = СтрШаблон("%1.%2", ОписаниеКласса.Класс, ИмяОбъекта);
...
&НаСервере
Функция ДанныеЗаполненияОбъекта(ОписаниеКласса, Знач ИмяОбъекта)
	
	ВЕдинственномЧисле = ПереводНаСервере("МножественноеРусский", "ЕдинственноеРусский");
	ИмяМетаданных = СтрШаблон("%1.%2", ВЕдинственномЧисле[ОписаниеКласса.Класс], ИмяОбъекта);
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ИмяМетаданных);
	
	Если ОписаниеКласса.Класс = "ОбщиеМодули"
		И ОбъектМетаданных <> Неопределено Тогда
		ОписаниеКласса.ШаблонОписания = ОписаниеКласса.ШаблонОписания + "_Расширяемый";
	КонецЕсли;
	...
	Если ОбъектМетаданных <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, ОбъектМетаданных);
	КонецЕсли;
...
&НаСервере 
Функция НовоеОписаниеПодчиненногоОбъекта(РодительскийКласс, РодительскийОбъект, ИмяОбъекта)
	
	ОписаниеРодительскогоКласса = ОписаниеКласса(РодительскийКласс);
	
	Если ОписаниеРодительскогоКласса.Формы 
		И Метаданные[РодительскийКласс][РодительскийОбъект].Формы.Найти(ИмяОбъекта) <> Неопределено Тогда
		
		Возврат НовоеОписаниеФормы(ОписаниеРодительскогоКласса, РодительскийОбъект, ИмяОбъекта);
...
&НаСервере
Функция НовоеОписаниеФормы(ОписаниеРодительскогоКласса, РодительскийОбъект, ИмяОбъекта)
	...
	ФайлОписания = СтрШаблон("%1%2",
		БазовыйПуть,
		СтрЗаменить(ОписаниеКлассаФормы.ФайлОписания, "{Объект}", ИмяОбъекта));
	...
	ПоАнглийски = ПереводНаСервере("ЕдинственноеРусский", "ЕдинственноеАнглийский");
	
	МетаданныеФормы = Метаданные[ОписаниеРодительскогоКласса.Класс][РодительскийОбъект].Формы.Найти(ИмяОбъекта);
	ТипФормы = ПоАнглийски[Строка(МетаданныеФормы.ТипФормы)];
	
	ОписаниеПодчиненногоОбъекта.Вставить("Тип", "Форма");
	ОписаниеПодчиненногоОбъекта.Вставить("ТипФормы", ТипФормы);
...
&НаКлиенте
Асинх Функция ДополнитьДанныеЗаполненияОписаниемКонфигурации(ДанныеЗаполнения, КлючиПодчиненныхОбъектов)
	
	ОписаниеОбъектаКонфигурации = ДанныеЗаполнения["Конфигурация."];
	
	Если ОписаниеОбъектаКонфигурации = Неопределено Тогда
		ОписаниеОбъектаКонфигурации = НовоеОписаниеОбъекта("Конфигурация", "");
	КонецЕсли;
	
	Если КлючиПодчиненныхОбъектов.Найти("Конфигурация.") <> Неопределено Тогда
		КлючиПодчиненныхОбъектов.Удалить(КлючиПодчиненныхОбъектов.Найти("Конфигурация."));
	КонецЕсли;
	
	ОписаниеОбъектаКонфигурации.ДанныеЗаполнения.Версия = ВерсияПрограммы();
	ОписаниеОбъектаКонфигурации.ДанныеЗаполнения.ПодчиненныеОбъекты = ОписаниеПодчиненныхОбъектовКонфигурации(КлючиПодчиненныхОбъектов);
	ОписаниеОбъектаКонфигурации.ДанныеЗаполнения.Язык = "Русский";
	ОписаниеОбъектаКонфигурации.ДанныеЗаполнения.РежимСовместимости = РежимСовместимости();
	
	ДанныеЗаполнения.Вставить("Конфигурация.", ОписаниеОбъектаКонфигурации);
...
Асинх Процедура СформироватьФайлМодуляОбъекта(ОписаниеОбъекта)
	...
	Для Каждого ПодчиненныйОбъект Из ОписаниеОбъекта.ПодчиненныеОбъекты Цикл
		...
		Если ОписаниеПодчиненногоОбъекта.Тип = "Форма" Тогда
			...
			ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, ОписаниеПодчиненногоОбъекта);
...
Функция МодульНаКлиенте(Модуль)
	...
	МетаданныеМодуля = Метаданные.ОбщиеМодули.Найти(Модуль);
	
	Возврат МетаданныеМодуля <> Неопределено И МетаданныеМодуля.КлиентУправляемоеПриложение;
...
&НаСервере
Функция ПереводНаСервере(ИсходнаяФорма, ТребуемаяФорма)
...
Функция ВыполнитьPowerShellСкрипт(ФайлСкрипта, Аргументы = Неопределено)
	...
	Результат = Оболочка.Run(Команда, 0, Истина);
...
Асинх Функция ЗапуститьОбработку()
	...
	Ждать СоздатьФайлНастройки();
	Ждать СоздатьФайлСкрипта();
...
&НаКлиенте
Асинх Функция СоздатьФайлНастройки()
...
&НаКлиенте
Асинх Функция СоздатьФайлСкрипта()
